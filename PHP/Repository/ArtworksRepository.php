...
class ArtworksRepository {
    ...
    public function postPainting($author, $title, $description, $years, $technique, $dimensions, $loan, $image, $user) {
        return $this->dbConnection->executeQuery("INSERT INTO Opere (ID, Autore, Titolo, Descrizione, Anni, Stile, Tecnica, Dimensioni, Prestito, Immagine, Utente) VALUES (NULL, '$author', '$title', '$description', '$years', '$historicalPeriod', 'Pittura', '$technique', '$dimensions', $loan, '$image', $user);");
    }

    public function postSculture($author, $title, $description, $years, $material, $dimensions, $loan, $image, $user) {
        return $this->dbConnection->executeQuery("INSERT INTO Opere (ID, Autore, Titolo, Descrizione, Anni, Stile, Materiale, Dimensioni, Prestito, Immagine, Utente) VALUES (NULL, '$author', '$title', '$description', '$years', '$historicalPeriod', 'Scultura', '$material', '$dimensions', $loan, '$image', $user);");
    }
    ...
    public function updatePainting($id, $author, $title, $description, $years, $technique, $dimensions, $loan, $image, $user) {
        return $this->dbConnection->executeQuery("UPDATE Opere SET Autore='$author', Titolo=$title'', Descrizione='$description', Anni='$years', Tecnica='$technique', Dimensioni='$dimensions', Prestito='$loan', Immagine='$image', Utente='$user' WHERE ID=$id;");
    }

    public function updateSculture($id, $author, $title, $description, $years, $material, $dimensions, $loan, $image, $user) {
        return $this->dbConnection->executeQuery("UPDATE Opere SET Autore='$author', Titolo=$title'', Descrizione='$description', Anni='$years', Materiale='$material', Dimensioni='$dimensions', Prestito='$loan', Immagine='$image', Utente='$user' WHERE ID=$id;");
    }
