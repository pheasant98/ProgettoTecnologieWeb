...
if ($event_count > 0) {
    ...
    if ($page > 1) {
        $navigation_events_buttons .= '<a id="buttonBack" class="button" href="?page=' . ($page - 1) . '&amp;filterType=' . $filter_type . '" title="Torna agli eventi precedenti" role="button" aria-label="Torna agli eventi precedenti"> &lt; Precedente</a>';
    }

    if (($page * 5) < $event_count) {
        $navigation_events_buttons .= '<a id="buttonNext" class="button" href="?page=' . ($page + 1) . '&amp;filterType=' . $filter_type . '" title="Vai agli eventi successivi" role="button" aria-label="Vai agli eventi successivi"> Successivo &gt;</a>';
    }
    ...
    if ($number_pages > 1) {
        if ($page === 1) {
            $skip_events = '<p class="skipDown">Ti trovi a pagina ' . $page . ' di ' . $number_pages . ':' . '</p> <a class="disable" href="#buttonNext">vai ai pulsanti di navigazione</a>';
        } else {
            $skip_events = '<p class="skipDown">Ti trovi a pagina ' . $page . ' di ' . $number_pages . ':' . '</p> <a class="disable" href="#buttonBack">vai ai pulsanti di navigazione</a>';
        }
    } else {
